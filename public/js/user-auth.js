// JavaScript for user authentication forms (login and registration/signup)

//ensure DOM content loaded before any JS executes
document.addEventListener('DOMContentLoaded', () => {
    const signupForm = document.getElementById('signup-form');
    const loginForm = document.getElementById('login-form');

    //allows cleanup of errors once user edits (to revalidate)
    ['input', 'change'].forEach(evt => {

        //add event listener to set custom error messages
    ...
    if (signupForm) {
        //conduct custom client-side validation once "submit" is clicked
        signupForm.addEventListener('submit', function(e) {

            //set up objects to validate
            ...
            //booleans used for error propagation
            let hasError = false;
            let passwordError = false;
            
            //check domain of the email address
            ...
            //check confirmation if password is valid (passwordError = false)
            //otherwise skip confirmation check since there is already a password error
            ...
            if (hasError) {
                //prevent form submission and error propagation
                e.preventDefault(); 
                e.stopPropagation(); 
                
                //show earliest error that is triggered
    ...
    if(loginForm) {
        //basic client-side validation similar to "signupForm"
